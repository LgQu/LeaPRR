...
from model.func import sparsemax
...
class Attention(nn.Module):
    ...
    def forward(self, x, A_mat=None, ori_sim=None):
        B, N, C = x.shape
        ...
        def separate():
            if A_mat is None:
                scale = self.scale
                attn = (q @ k.transpose(-2, -1)) * scale # (B, head, N, N)
                attn = attn.softmax(dim=-1) # (B, head, N, N)
                attn = self.attn_drop(attn)
            else:
                attn_gcn = A_mat.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
                attn = attn_gcn
            return attn
...
class Block(nn.Module):
    ...
    def forward(self, x, A_mat=None, ori_sim=None):
        x = x + self.drop_path(self.attn(self.norm1(x), A_mat=A_mat, ori_sim=ori_sim))
