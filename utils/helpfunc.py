...
class WarmupCos_Scheduler(object):
    ...
    def step(self):
        ...
        cur_lr = self.lr_schedule[self.iter]
        self.iter += 1
        return cur_lr
...
class WarmupStep_Scheduler(object):
    def __init__(self, optimizer, warmup_epochs, warmup_lr, num_epochs, base_lr, iter_per_epoch, step_size, gamma):
        self.base_lr = base_lr
        warmup_iter = iter_per_epoch * warmup_epochs
        warmup_lr_schedule = np.linspace(warmup_lr, base_lr, warmup_iter)
        decay_iter = iter_per_epoch * (num_epochs - warmup_epochs)
        step_lr_schedule = np.ones(decay_iter)
        exponent = 0
        for critical_point in range(0, decay_iter, int(step_size * iter_per_epoch)):
            step_lr_schedule[critical_point:] = base_lr * gamma ** (exponent)
            exponent += 1
...
class MetricLogger(object):
    ...
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            if name == 'LR':
                loss_str.append("{}: {:.2g}".format(name, meter.val))
            else:
                loss_str.append("{}: {:.4f} ({:.4f})".format(name, meter.val, meter.avg))
