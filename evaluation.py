...
def i2t(sims, npts=None, return_ranks=False):
    ...
    """
    # print('Images->Text (Image Annotation) ...')
    npts = sims.shape[0]
    ranks = np.zeros(npts)
    top1 = np.zeros(npts)
...
def t2i(sims, npts=None, return_ranks=False):
    ...
    sims: (N, 5N) matrix of similarity im-cap
    """
    # print('Text->Images (Image Search) ...')
    npts = sims.shape[0]
...
def evalrank_single(sims, data_path=None, split='test', fold5=False):
    if not fold5:
    ...
    return r[0], r[1], r[2], ri[0], ri[1], ri[2], rsum
